...
@router.get("/utmify/campaigns/escalados/{day}")
def get_all_campaigns(day: str):
    name_contains = None

    try:
        response = get_campaigns(
            day=day, name_contains=name_contains, products=None, level="campaign"
        )

        if response.status == 400:
            print(response.message)
            return ReportResponse(
                report_title="Write Utmify Campaigns - Error",
        ...
        worksheet_index = search_worksheet_index(
            "DB_3.0", "1kYakvWtJ-2G1Vu-ylxb4qYCzSoozMunz", "RAW-CAMPAIGNS-ESCALADOS"
        )
        ...
        return ReportResponse(
            report_title="Write Utmify Campaigns - Success",
            generated_at=datetime.now(),
            message="Os dados dos anúncios foram escritos com sucesso!",
            status=200,
        )
