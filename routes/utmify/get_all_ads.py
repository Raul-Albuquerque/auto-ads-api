...
@router.get("/utmify/all/{day}")
def get_all_ads(day: str):
    name_contains = None

    try:
        response = get_campaigns(
            day=day, name_contains=name_contains, products=None, level="ad"
        )
        ...
        worksheet_index = search_worksheet_index(
            "DB_3.0", "1kYakvWtJ-2G1Vu-ylxb4qYCzSoozMunz", "RAW"
        )
        ...
        return ReportResponse(
            report_title="Write Utmify Ads - Success",
            generated_at=datetime.now(),
