...
def write_ads_escalados_report():
  ...
  try:
    ...
    try:
      ...
      print(e)
    return ReportResponse(report_title="Write ads escalados report - Success", generated_at=datetime.now(), message=f"Relatórios de ads escalados escritos com sucesso!", status=200)
  except Exception as e:
    return ReportResponse(report_title="Write ads escalados report - Error", generated_at=datetime.now(), message=f"Error: {str(e)}", status=400)