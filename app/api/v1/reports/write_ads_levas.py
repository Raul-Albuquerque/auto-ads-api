...
from app.models.payload_models import AdsLevasReport
...
@router.post("/levas")
async def write_ads_levas(filters: AdsLevasReport, api_key: str = Depends(get_api_key)):
    try:
        if filters.active_offer != "all":
            try:
                ads_levas_report(active_offer=filters.active_offer)
            except Exception as e:
                print(e)
        else:
            try:
                all_ads_levas_report()
            except Exception as e:
                print(e)
